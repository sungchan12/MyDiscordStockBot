...
Discord에서 `/주식` 명령으로 등록한 국내·해외 종목의 실시간 시세를 조회해주는 Kotlin 봇입니다.
...
- **국내(KR)·해외(US) 동시 지원** — 한국거래소 6자리 코드, 나스닥/NYSE 티커
- **실시간 환율(USD→KRW)** 자동 반영
- **종목별 상승/하락 메시지** — `upMessage`(상승 시), `downMessage`(하락 시)를 코드블록으로 전송
- **설정 파일 기반** — 종목 추가/삭제는 `stocks.yaml`만 수정하고 봇 재시작 (재빌드 불필요)
- 시세는 [Yahoo Finance](https://finance.yahoo.com) v8 chart API로 조회 (API 키 불필요)
...
1. `.env.example` 을 복사해 `.env` 를 만들고 값을 채웁니다. (`.env` 는 git에 올라가지 않습니다)
